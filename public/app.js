...
function renderGamePage(slug) {
  ...
  $('#app').innerHTML = `<section class="game-page">
    <div class="game-page-hero tone-${meta.tone}">
      ...
      <div class="game-page-hero-main"><div class="large-game-icon ${meta.imageType==='wordmark'?'wordmark-frame':''}"><img class="${meta.imageType==='wordmark'?'wordmark-icon':''}" src="${meta.image}" alt="${game} ${meta.imageType==='wordmark'?'wordmark':'icon'}" onerror="this.src=fallbackImage('${game}')"></div><div><span class="section-kicker">${meta.publisher}</span><h1>${game}</h1><p>${meta.subtitle}</p><div class="hero-tags"><span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m13 2-9 12h7l-1 8 10-13h-7l1-7Z"/></svg>${items.length} packages</span><span><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="10" width="16" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>Secure checkout coming soon</span><span><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18"/></svg>${regions.join(' • ') || 'Global'}</span></div></div></div>
    </div>

    <div class="product-page-grid">
      <div class="packages-column">
        <div class="page-heading"><div><span class="section-kicker">AVAILABLE PACKAGES</span><h2>Explore the options.</h2><p>Compare package details and prices. Secure checkout will open when payments and order fulfillment are ready.</p></div><a class="mini-back" onclick="goHome()">Browse another game →</a></div>
...
function openBuy(id) {
  ...
  $('#modalBody').innerHTML = `<div class="checkout-head"><img class="checkout-icon ${meta.imageType==='wordmark'?'wordmark-icon':''}" src="${meta.image}" alt="${p.game} ${meta.imageType==='wordmark'?'wordmark':'icon'}" onerror="this.src=fallbackImage('${p.game}')"><div><span class="section-kicker">ORDER DETAILS</span><h2>${p.game}</h2><p>${p.title} • ${p.region}</p></div></div>
    <div class="checkout-body"><div class="preview-notice"><span aria-hidden="true">i</span><div><strong>Ordering is not open yet</strong><p>We’re preparing verified payments and secure delivery. No player details or payment information are collected here.</p></div></div>
    <div class="checkout-section preview-fields"><div class="checkout-section-title"><span>1</span><div><b>Player information</b><small>Required details for this package</small></div></div><div class="info-fields">${(p.fields||[]).map(f=>`<span>${fieldLabel(f)}</span>`).join('')}</div></div>
    <div class="checkout-section"><div class="checkout-section-title"><span>2</span><div><b>Payment</b><small>Payment methods will appear after secure checkout is ready.</small></div></div><div class="payment-coming-soon">Checkout setup in progress</div></div>
    <div class="checkout-total"><div><small>PACKAGE PRICE</small><b>${p.title}</b></div><strong>${money(p.displayPrice,currency)}</strong></div>
    <button class="checkout-submit checkout-disabled" type="button" disabled aria-disabled="true">Ordering opens soon</button></div>`;
}
